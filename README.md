...
- **全屏防截图水印**：所有页面（含登录/注册/404）叠加 `notellm.au1bhi.com` 平铺水印（`WATERMARK_TEXT` 可配置、服务端下发，`WATERMARK_ENABLED` 可整体开关）。双层冗余（`index.html` 静态层 + React 全文档层，覆盖整页截图折叠线以下）+ 观察器/定时器即时重建 + 内联 `!important` 样式压过注入样式表，文字只存在于 SVG data-URI、无 DOM 文本节点可选删。经红蓝队对抗与真实浏览器验证：删除、`display/opacity/transform/filter/clip-path` 隐藏、样式表注入、挪进变换祖先容器等手法均无法让截图失去水印。
