...
- **安全实践可迁移**：包含资源所有权校验、API Key 加密、配额原子预留、邮箱验证、SSRF 防护和引用白名单等可独立参考的实现。
...
`install.sh` 会引导生成必要配置。生产环境如需邮箱验证、找回密码和可选学习提醒，需要配置 SMTP；未配置时不会发送邮件。详细参数以 `bash install.sh --help` 和 [.env.example](.env.example) 为准。
