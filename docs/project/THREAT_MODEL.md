...
| 可信 | 运营者环境变量与 Compose 配置 | 可信 | `SECRET_KEY`、服务端 `LLM_*` / `EMBEDDING_*`、可选 `SERVER_PROVIDER_PROXY_URL`、SMTP。 |
...
| 服务端免费额度 | **D / E** 并发击穿月额度、用 BYOK 却仍消耗运营者配额、未验证邮箱刷服务端调用 | 计费维度在模型调用前 `reserve_usage` 原子预留（`INSERT … ON CONFLICT` + `WHERE` 上限，行锁串行化），调用后再 `settle_usage` 按「实际 − 预留」结算（可退还）。自备 Key 的维度 `quota` 为 `None`，预留量为 0，不计入服务端免费额度。当邮件后端开启且本次将走服务端聊天或嵌入时，`_require_verified_for_server_usage` 拒绝未验证邮箱；BYOK 不经过该门闩。删除账户会把用量写入按规范邮箱索引的墓碑，再注册同址不能刷新当月额度。 | `backend/app/services/usage.py`（`reserve_usage`、`settle_usage`、`_require_verified_for_server_usage`、`quota_status`、`save_usage_tombstone`）；`backend/tests/api/routes/test_usage.py`：`test_reserve_usage_is_atomic_and_stops_at_quota`、`test_reserve_with_own_key_is_unlimited`、`test_chat_quota_exhausted_returns_error_event`；`backend/tests/api/routes/test_users.py`：`test_delete_and_reregister_preserves_allowance` | 套件默认关闭 SMTP（`backend/tests/conftest.py` 的 `_disable_email_verification_gate`），未验证门闩在测试中不触发，目前没有「未验证 + 邮件开启 → 拒用服务端额度」的专项用例。预留裕量（`CHAT_RESERVE_MARGIN`）会限制突发并发。未配置邮件的部署不启用该门闩。 |
...
| Provider 密钥与 `SECRET_KEY` | **I** 密钥下发到浏览器、数据库明文存放、JWT 被盗后长期有效 | 服务端密钥只从环境变量读取，浏览器不直接调模型。用户 BYOK 写入时 `encrypt_secret`（Fernet，密钥由 `SECRET_KEY` 的 SHA-256 派生）落库；读回接口经 `mask_secret` 只返回掩码（如 `sk-m***3456`），从不回传明文。改密更新 `password_changed_at`，使旧 access token 立即失效。 | `docs/project/ARCHITECTURE.md`「安全边界」；`backend/app/core/security.py`（`encrypt_secret` / `decrypt_secret` / `create_access_token`）；`backend/app/api/routes/users.py`（`_provider_settings_public`、`upsert_user_provider_settings`）；`backend/app/services/provider_settings.py`（`mask_secret`）；`backend/app/api/deps.py`（`pwd` 快照校验）；`backend/tests/api/routes/test_provider_settings.py`：`test_encrypt_secret_round_trip`、`test_upsert_stores_encrypted_and_returns_masked`；`backend/tests/api/routes/test_users.py`：`test_access_token_without_pwd_snapshot_is_rejected` | `SECRET_KEY` 泄露可解密全部已存 BYOK。掩码仍暴露首尾各 4 个字符。明文密钥在出站请求期间驻留进程内存。无独立 KMS / 密钥轮换流程。 |
