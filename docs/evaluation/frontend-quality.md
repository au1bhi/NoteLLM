...
- token 过期、验证错误或认证请求失败后会清空并重置组件；配置或脚本加载失败时提交按钮保持禁用并提供重试。脚本加载超过 12 秒会进入可重试错误态，旧 widget 的延迟回调不能覆盖新实例 token。
- 登录、注册、找回、重置与验证邮件操作使用 15 秒请求超时；提交锁在成功、API 错误、断网和超时后释放，同一渲染帧的双击不会发出重复请求。断网与超时统一显示中文且保留输入。429 会读取 CORS 暴露的 `Retry-After` 并显示精确秒数，不自动重提业务请求；普通查询最多重试三次，401、403、429 查询不重试。
- 认证布局在 320、375 和 768px 宽度下为 flexible Turnstile 保留可用宽度；超长邮箱允许在窄屏内断行。
...
| 脚本永久挂起 | 阻止脚本 load/error 事件 | 12 秒后结束加载态，显示失败与重试按钮 |
| 重复提交 | 快速双击按钮或连续按 Enter | 只发出一次登录、注册、找回、重置或重发请求 |
| 请求永久挂起 | 让认证 API 建连后不返回 | 15 秒后显示中文超时提示，保留输入并允许重试 |
| 请求限流 | 在窗口内超过端点限额 | 显示后端通用 429 文案和准确等待秒数，不自动重提；窗口结束后可由用户重试 |
| 数据库/迁移异常 | 停止数据库或使用未迁移 schema | 显示可恢复 503；运维 readiness 同时失败，修复后无需刷新应用进程即可重试 |
