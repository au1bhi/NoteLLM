...
- 登录、注册、找回、重置与验证邮件操作使用 15 秒请求超时；提交锁在成功、API 错误、断网和超时后释放，同一渲染帧的双击不会发出重复请求。429 会读取 CORS 暴露的 `Retry-After` 并显示精确秒数。
...
| 请求限流 | 在窗口内超过端点限额 | 显示后端通用 429 文案和准确等待秒数，窗口结束后可恢复 |
