...
服务端免费额度为对话 10 万 token、嵌入 30 万字符，按自然月惰性重置。统一的 `usage_reservation` 在调用前用 `reserve_usage` 原子预留，成功按实际用量结算；异常先回滚当前业务事务，再只退还本次真实预留量。自备 Key 的对应维度预留量为 0，且上下文忽略该维度的实际值，避免「幽灵退款」抹掉无关计数。

用户 BYOK 用 Fernet 加密落库；HKDF 从 `SECRET_KEY` 分别派生 JWT 签名与 Fernet 加密子密钥，避免两个用途复用同一有效密钥。升级期间仍可读取旧 SHA-256 Fernet 密文与旧 raw-secret JWT，新写入和新签发只用 HKDF 子密钥。读回接口只返回掩码，从不回传明文。访问 JWT、密码重置令牌和换邮令牌都绑定 `password_changed_at` 的微秒快照（声明名 `pwd`）。改密或管理员重置会清空 `pending_email`，并使这三类旧令牌立即失效。删除账户把当月用量写入按规范邮箱索引的墓碑，同址再注册不能刷新额度。

残余风险：`SECRET_KEY` 泄露可解密全部已存 BYOK；掩码仍暴露首尾各 4 个字符；明文密钥在出站请求期间驻留进程内存。

## 5.5 公开认证防滥用

登录、注册和找回密码可启用 Cloudflare Turnstile。站点密钥与服务端密钥必须成对配置；前端只从公开元数据取得站点密钥，并用 `X-Turnstile-Token` 提交一次性 token，secret 不进入浏览器。启用后缺 token、验证失败或 Cloudflare 验证服务异常均失败关闭。

认证固定窗口与收件人邮件冷却写入 PostgreSQL `rate_limit_bucket`，因此四个 API worker 共享同一个原子计数，不再各自放大额度。新身份经事务 advisory lock 受活跃桶硬上限约束，IPv6 按 `/64` 聚合。后端关闭 uvicorn 的代理头重写并自行解析原始 TCP peer，只接受 `TRUSTED_PROXY_CIDRS` 直接代理提供的 XFF，并从右向左剥离可信跳；应用不直接消费可由公网客户端携带的 `CF-Connecting-IP`。Cloudflare Tunnel 只连接宿主机环回映射的 nginx 专用端口，该端口把边缘验证头转换为 XFF、清除原头；普通 Traefik 端口只追加实际链路。这样同时避免伪造头绕过与 Tunnel 单桶。残余风险是 Turnstile 与限流分别依赖 Cloudflare 和 PostgreSQL 可用性，容量耗尽时新身份会失败关闭。
